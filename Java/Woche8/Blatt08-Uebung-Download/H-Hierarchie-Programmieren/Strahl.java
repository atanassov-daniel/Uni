public sealed class Strahl extends Gerade permits Strecke {
    ...
    */
    public Strahl(Punkt a, Punkt b) {
        super(a, b);
        // wenn p1 von der Gerade(es wird ja normiert je anchdem ob a oder b linkt steht) mit das Parameter a uebereinstimmt, dann beginnt der Strahl in p1; wenn p1 != a ist, dann ist der Anfangspunkt a des Strahls von dem Konstruktor von Gerade auf p2 gesetzt worden, also muss hier beginntInP1 sagen, dass der Anfangspunkt a des Strahls eigentlich als p2 gespeichert worden ist
        beginntInP1 = this.getP1().equals(a) ? true : false;
        //TODO Achten Sie darauf, korrekt mit möglicherweise auftretenden Exceptions umzugehen.
    }
    ...
    // sinnvolle Ausgabe nach dem Vorbild der zu überschreibenden Methode der Oberklasse. Im zurückgegebenen String sollte deutlich werden, wo der Strahl beginnt und welchen Punkt der Strahl lediglich passiert.
    @Override
    public String toString() {
        return "Strahl beginnt von " + (this.startsFromp1() ? this.getP1() : this.getP2()) + " und passiert durch "
                + (this.startsFromp1() ? this.getP2() : this.getP1());
    }
    ...
    /* Implementieren Sie dann unter Nutzung der zu überschreibenden Methode aus der Oberklasse außerdem die Methode boolean equals(Objekt obj). */ //! das sollte glaube ich heissen, dass ich irgendwo in meine Implementierung hier super.equals() aufrufen sollte
